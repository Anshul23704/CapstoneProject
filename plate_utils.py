...
def get_tilted_bbox(plate_crop: np.ndarray, original_bbox: BBox) -> np.ndarray:
    """
    Given a plate crop and its original axis-aligned bbox in the full frame,
    returns the 4 coordinates of the minimum area rotated rectangle that
    tightly bounds the license plate contour. If contour detection fails,
    returns the original 4 corners.
    Returns: numpy array of shape (4, 2) dtype int32
    """
    ...
    try:
        ...
        # shift box back to original coordinates
        box[:, 0] += x1
        box[:, 1] += y1
