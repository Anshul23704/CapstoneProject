            ...
            if self.cfg.save_crops and self.cfg.crops_dir:
                try:
                    ...
                    if roi is not None and roi.size > 0:
                        ...
                        crop = context_img[ly1:ly2, lx1:lx2]
                        tilted_box = get_tilted_bbox(crop, (lx1, ly1, lx2, ly2))
                        cv2.polylines(context_img, [tilted_box], True, (0, 255, 0), 2)
                        ...
                        if self.cfg.high_conf_crops_dir and frame_conf >= self.cfg.high_conf_threshold:
                            os.makedirs(self.cfg.high_conf_crops_dir, exist_ok=True)
                            cv2.imwrite(os.path.join(self.cfg.high_conf_crops_dir, f"{prefix}_plate_bilateral.png"), plate_bilateral)
                            cv2.imwrite(os.path.join(self.cfg.high_conf_crops_dir, f"{prefix}_plate_adaptive.png"), plate_adaptive)
                            cv2.imwrite(os.path.join(self.cfg.high_conf_crops_dir, f"{prefix}_context.png"), context_img)
                except Exception as exc:
